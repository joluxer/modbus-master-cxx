/*
 * ModbusRtu.cpp
 *
 *  Created on: 15.02.2016
 *      Author: lode
 */

#include "ModbusRtu.h"

#include <cmath>

namespace Modbus
{

Rtu::Rtu()
: pduCrc(~0), crcTxOffset(0)
{}

//Rtu::~Rtu()
//{
//  // Auto-generated destructor stub
//}

#ifdef MODBUS_CRC_ALGORITHM_TABLE
/* Table of CRC values for high–order byte */
static unsigned char crcTableHi[256] =
{
    
};

/* Table of CRC values for low–order byte */
static unsigned char crcTableLo[] =
{
    
};

// this CRC function comes from Modbus_over_serial_line_V1_02.pdf
void Rtu::updateCrc(uint8_t const* data, unsigned len)
{
  uint8_t crcHi = (pduCrc >> 8);            /* high byte of CRC initialized */
  uint8_t crcLo = (pduCrc >> 0);            /* low byte of CRC initialized */
  unsigned tableIndex;                      /* will index into CRC lookup table */

  while (len--)                             /* pass through message buffer */
  {

    tableIndex = crcLo ^ *data++;           /* calculate the CRC */
    crcLo = crcHi ^ crcTableHi[tableIndex];
    crcHi = crcTableLo[tableIndex];
  }

  pduCrc = (crcLo << 0 | crcHi << 8);
}
#endif

#ifdef MODBUS_CRC_ALGORITHM_BITSHIFT
// this CRC function comes from modbus_basics_d.pdf
void Rtu::updateCrc(uint8_t const* data, unsigned len)
{
  constexpr unsigned int Poly16 = 0xA001;
  unsigned int lsb, i;

  while (len)                             /* pass through message buffer */
  {
    pduCrc ^= uint16_t(*data);
    for (i = 0; i < 8; ++i)
    {
      lsb = (pduCrc & 0x0001);
      pduCrc = pduCrc >> 1;
      if (lsb)
        pduCrc ^= Poly16;
    }
    ++data;
    --len;
  }
}
#endif

uint8_t Rtu::runTx(ByteStream* line, const ::std::unique_ptr<Txn>& runningTxn, uint8_t offset)
{
  PduConstDataBuffer txBuffer;
  uint8_t done = 0;

  runningTxn->getTxData(txBuffer, offset);

  if (txBuffer.length > 0)
  {
    done = line->writeBlock(txBuffer.data, txBuffer.length);
    updateCrc(txBuffer.data, done);
  }

  return done;
}

uint8_t Rtu::startTx(ByteStream* line, const ::std::unique_ptr<Txn>& runningTxn)
{
  resetCrc();
  crcTxOffset = 0;

  auto id = runningTxn->getSlaveId();
  updateCrc(&id, 1);
  line->write(id);

  return runTx(line, runningTxn, 0);
}

void Rtu::startRx()
{
  pduCrc = ~0;
}

unsigned Rtu::runRx(ByteStream* line, uint8_t* buffer, const size_t bufferSpace)
{
  auto count = line->readBlock(buffer, bufferSpace);

//  printf("\n%s: %d\n", __PRETTY_FUNCTION__, count);

  return count;
}

uint32_t Rtu::getTxGuardTime_ms(uint32_t baudrate)
{
  if (19200 >= baudrate)
  {
    auto q = div(long(35000), long(baudrate));

    if (q.rem > 0)
      ++q.quot;

    return q.quot;
  }

  return 2;
}

uint32_t Rtu::getSplitLimit_ms(uint32_t baudrate)
{
  if (19200 >= baudrate)
  {
    auto q = div(long(35000), long(baudrate));

    if (q.rem > 0)
      ++q.quot;

    return q.quot;
  }

  return 1;
}

PduConstDataBuffer Rtu::getPdu(uint8_t* buffer, uint8_t bufferFill, uint8_t offset)
{
  if ((offset + 2) <= bufferFill)
  {
    PduConstDataBuffer pdu{buffer + offset, uint8_t(bufferFill - offset - 2)};
    return pdu;
  }

  return PduConstDataBuffer{buffer + offset, 0};
}

} /* namespace Modbus */
